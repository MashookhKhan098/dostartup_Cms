...
export default function StartBusinessPage({ defaultEntity = "Trust" }) {
  const router = useRouter();
  const [activeEntity, setActiveEntity] = useState(defaultEntity);

  const entities = [
    { name: "Section 8", path: "/section-8-company-registration" },
    { name: "Trust", path: "/trust-registration" },
    { name: "Public Limited", path: "/public-limited-company" },
    { name: "Producer", path: "/producer-company-registration" },
    { name: "Indian Subs.", path: "/indian-subsidiary" },
  ];

  const descriptions = {
    "Section 8": "A Section 8 Company is a non-profit organization registered to promote charitable objectives like education, arts, science, or social welfare. It enjoys tax exemptions and requires no minimum capital for incorporation.",
    "Trust": "A Trust is a legal entity created for charitable, religious, or social purposes. It operates under a trust deed and provides tax benefits while being managed by trustees for the benefit of beneficiaries.",
    "Public Limited": "A Public Limited Company can raise capital from the public through stock exchanges. It requires minimum 7 shareholders, 3 directors, and offers shares to the public with higher compliance and transparency requirements.",
    "Producer": "A Producer Company is formed by primary producers like farmers, cultivators, or artisans for mutual benefit. It combines features of cooperative societies and private limited companies with relaxed compliance norms.",
    "Indian Subs.": "An Indian Subsidiary is a company incorporated in India where the majority stake is held by a foreign parent company. It operates as a separate legal entity and is ideal for foreign companies entering the Indian market.",
  };
              ...
              <div className="flex border-b border-gray-200 min-w-max">
                {entities.map(({ name, path }) => (
                  <button
                    ...
                    className={`px-5 py-4 text-xs sm:text-sm font-semibold whitespace-nowrap transition-all relative ${
                      activeEntity === name
