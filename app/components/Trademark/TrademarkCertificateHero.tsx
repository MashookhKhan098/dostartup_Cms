...
export type TrademarkCertificateHeroProps = {
  heading?: string;
...
export default function TrademarkCertificateHero({
  heading = "Trademark Registration",
  headingHighlight = "Certificate",
  description = "Get your official Trademark Registration Certificate. Essential for legal protection and brand ownership proof.",
  features,
  buttonText,
}: TrademarkCertificateHeroProps) {
  ...
  useEffect(() => {
    ...
    const fetchPricing = async () => {
      try {
        const filter = JSON.stringify({ category: { "$regex": "^trademark-registration-certificate$", "$options": "i" } });
  ...
  const handlePayment = async () => {
    ...
    try {
      const res = await fetch("/api/create-order", {
        ...
        body: JSON.stringify({ amount, packageName: "Trademark Certificate" }),
      });
      ...
      const options = {
        ...
        currency: "INR",
        name: "DoStartup",
        description: "Trademark Certificate",
        order_id: orderId,
        handler: async function (response: any) {
          ...
          const whatsappMsg =
            `🔔 *New Trademark Registration Certificate*\n\n` +
            `👤 *Name:* ${formData.name}\n` +
            `📞 *Phone:* ${formData.phone}\n` +
            ...
            `I have successfully completed the payment. Please proceed with my Trademark Certificate.`;

          const waUrl = `https://wa.me/${process.env.NEXT_PUBLIC_WHATSAPP_NUMBER || "919999644807"}?text=${encodeURIComponent(whatsappMsg)}`;

          // Save to Supabase (Table: trademark_certificate)
          try {
            const finalEmail = user?.email || formData.email || "noemail@trademark.com";
            const { error: dbError } = await supabase.from("trademark_certificate").insert({
              name: formData.name,
              email: finalEmail,
              phone_no: formData.phone,
            ...
            console.error("Failed to insert Trademark Certificate record:", dbErr);
          }
        ...
        <div className="flex flex-col lg:flex-row gap-8">
            ...
            <div className="flex flex-col md:flex-row gap-8">
              <div className="w-full md:w-64 flex-shrink-0">
                <div className="w-full h-48 bg-gradient-to-br from-[#C15F3C] to-[#A94E30] rounded-xl border border-[#E5E2DA] flex items-center justify-center">
                  <span className="text-white font-bold text-xl text-center px-4">Trademark Certificate</span>
                </div>
                <div className="mt-4 space-y-2">
                  {["Official Certification", "Brand Ownership Proof", "Legal Compliance"].map((item) => (
              ...
              <div className="flex-1 space-y-6">
                ...
                <div className="bg-[#F9F8F6] border border-[#E5E2DA] rounded-2xl p-6">
                  ...
                  <div className="space-y-3">
                    {(features || [
                      { text: "Fast-track certificate issuance" },
                      { text: "Verified government documentation" },
                      { text: "Legal proof of ownership" },
                      { text: "End-to-end expert support" }
                    ]).map((f, i) => (
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <button onClick={() => handleNeedHelpWhatsApp("Trademark Certificate")} className="text-[#C15F3C] hover:underline">Need Help?</button>
                </div>
              </div>
            </div>
          </div>
          ...
          <div id="registration-form" className="lg:w-96 bg-white rounded-2xl shadow-sm border border-[#E5E2DA] overflow-hidden self-start">
            <div className="bg-gradient-to-r from-[#C15F3C] to-[#A94E30] px-6 py-4">
              <h2 className="text-lg font-semibold text-white">Get Certificate</h2>
              <p className="text-sm text-[#F4F3EE]">Official proof of brand ownership</p>
            </div>
            ...
            <p className="px-6 pt-1 pb-0 text-[11px] text-[#B1ADA1]">
              {step === 1 ? "Step 1 of 2 – Contact Details" : "Step 2 of 2 – Certificate Details"}
            </p>
              ...
              <div className="p-8 space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-4 border-l border-[#E5E2DA] pl-6">
                    <h4 className="text-[10px] font-bold text-[#B1ADA1] uppercase tracking-wider">Certificate Details</h4>
                    <div className="space-y-2">
                      {[
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }}
              ...
              <div className="p-8 space-y-6 text-center">
                <p className="text-[#6F6B63] text-sm">Your certificate request for brand <span className="font-bold text-[#2F2E2B]">{formData.brand_name}</span> is being processed.</p>
