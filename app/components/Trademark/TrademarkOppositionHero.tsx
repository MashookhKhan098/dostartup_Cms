...
export type TrademarkOppositionHeroProps = {
  heading?: string;
...
export default function TrademarkOppositionHero({
  heading = "Trademark",
  headingHighlight = "Opposition",
  description = "Protect your brand rights by filing a formal opposition against conflicting trademark applications.",
  features,
  buttonText,
}: TrademarkOppositionHeroProps) {
  ...
  useEffect(() => {
    ...
    const fetchPricing = async () => {
      try {
        const filter = JSON.stringify({ category: { "$regex": "^trademark-opposition$", "$options": "i" } });
  ...
  const handlePayment = async () => {
    ...
    try {
      const res = await fetch("/api/create-order", {
        ...
        body: JSON.stringify({ amount, packageName: "Trademark Opposition" }),
      });
      ...
      const options = {
        ...
        handler: async function (response: any) {
          ...
          const whatsappMsg =
            `🔔 *New Trademark Opposition Filing*\n\n` +
            `👤 *Name:* ${formData.name}\n` +
            `📞 *Phone:* ${formData.phone}\n` +
            ...
            `I have successfully completed the payment. Please proceed with my Trademark Opposition Filing.`;

          const waUrl = `https://wa.me/${process.env.NEXT_PUBLIC_WHATSAPP_NUMBER || "919999644807"}?text=${encodeURIComponent(whatsappMsg)}`;

          // Save to Supabase (Table: trademark_opposition)
          try {
            const finalEmail = user?.email || formData.email || "noemail@trademark.com";
            const { error: dbError } = await supabase.from("trademark_opposition").insert({
              name: formData.name,
              email: finalEmail,
              phone_no: formData.phone,
            ...
            console.error("Failed to insert Trademark Opposition record:", dbErr);
          }
        ...
        <div className="flex flex-col lg:flex-row gap-8">
            ...
            <div className="flex flex-col md:flex-row gap-8">
              <div className="w-full md:w-64 flex-shrink-0">
                <div className="w-full h-48 bg-gradient-to-br from-[#C15F3C] to-[#A94E30] rounded-xl border border-[#E5E2DA] flex items-center justify-center">
                  <span className="text-white font-bold text-xl text-center px-4">Trademark Opposition</span>
                </div>
                <div className="mt-4 space-y-2">
                  {["Brand Protection", "Legal Challenge", "Expert Representation"].map((item) => (
              ...
              <div className="flex-1 space-y-6">
                <div className="inline-flex items-center gap-2 bg-white border border-[#E5E2DA] rounded-full px-3 py-1">
                  ...
                  <span className="text-xs font-medium text-[#C15F3C]">LEGAL EXPERTS</span>
                </div>
                ...
                <div className="bg-[#F9F8F6] border border-[#E5E2DA] rounded-2xl p-6">
                  ...
                  <div className="space-y-3">
                    {(features || [
                      { text: "Formal opposition drafting" },
                      { text: "Grounds for challenge analysis" },
                      { text: "Legal documentation support" },
                      { text: "Filing within statutory timelines" }
                    ]).map((f, i) => (
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <button onClick={() => handleNeedHelpWhatsApp("Trademark Opposition")} className="text-[#C15F3C] hover:underline">Need Help?</button>
                </div>
              </div>
            </div>
          </div>
          ...
          <div id="registration-form" className="lg:w-96 bg-white rounded-2xl shadow-sm border border-[#E5E2DA] overflow-hidden self-start">
            <div className="bg-gradient-to-r from-[#C15F3C] to-[#A94E30] px-6 py-4">
              <h2 className="text-lg font-semibold text-white">File Opposition</h2>
              <p className="text-sm text-[#F4F3EE]">Protect your brand's uniqueness</p>
            </div>
            ...
            <p className="px-6 pt-1 pb-0 text-[11px] text-[#B1ADA1]">
              {step === 1 ? "Step 1 of 2 – Contact Details" : "Step 2 of 2 – Opposition Details"}
            </p>
            ...
            <div className="p-6">
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="flex items-center justify-center gap-2 text-xs text-[#B1ADA1] pt-2">
                  ...
                  <span>Secure Gateway · Professional filing</span>
                </div>
              </form>
            </div>
          </div>
              ...
              <div className="p-8 space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-4 border-l border-[#E5E2DA] pl-6">
                    <h4 className="text-[10px] font-bold text-[#B1ADA1] uppercase tracking-wider">Opposition Details</h4>
                    <div className="space-y-2">
                      {[
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }}
              ...
              <div className="p-8 space-y-6 text-center">
                <p className="text-[#6F6B63] text-sm">Your opposition filing for brand <span className="font-bold text-[#2F2E2B]">{formData.brand_name}</span> is being processed.</p>
