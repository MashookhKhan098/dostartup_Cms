...
export default function DynamicHeroSection({
  ...
  const renderFormField = (field: FormField, index: number): React.ReactNode => {
    switch (field.type) {
              ...
              {((field as any).options || []).map((option: ColorOption, idx: number) => (
                <label key={idx} className="flex items-center gap-1.5 cursor-pointer">
                  <input
                    ...
                    value={option.hex}
                    className="w-3.5 h-3.5 text-amber-600 border-gray-300 rounded focus:ring-1 focus:ring-amber-600"
                  />
                  <span
      ...
      case "text":
        return (
          <p key={index} className="text-xs sm:text-sm text-gray-500">{field.content}</p>
        );
        ...
        <div className="grid lg:grid-cols-2 gap-6 lg:gap-8 items-start">
          ...
          <div className="space-y-5 sm:space-y-6">
            ...
            <div className="space-y-2">
              <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold tracking-tight">
                <span className="text-gray-900">{heading?.split(headingHighlight)[0]}</span>
                <span className="text-amber-700">
                  {headingHighlight}
                </span>
                <span className="text-gray-900">{heading?.split(headingHighlight)[1]}</span>
              </h1>
          ...
          <div className="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">
            ...
            <div className="p-4 sm:p-5">
              {/* Tab Description */}
              {tabDescriptions && activeTab && tabDescriptions[activeTab] && (
                <div className="bg-amber-50 p-3 sm:p-4 rounded-lg mb-4 border-l-4 border-amber-600">
