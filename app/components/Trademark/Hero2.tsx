  ...
  useEffect(() => {
    const checkUser = async () => {
      try {
        ...
        console.warn("Trademark/Hero2 initial auth check failed gracefully:", err.message);
      }
    };
    checkUser();
  }, []);
                ...
                <div className="bg-[#F9F8F6] border border-[#E5E2DA] rounded-2xl p-6">
                  ...
                  <div className="space-y-3">
                    {features?.map((feature, index) => (
                      <div key={index} className="flex items-center gap-3 group bg-white p-3.5 rounded-xl border border-[#E5E2DA] shadow-sm">
                        <div className="w-8 h-8 bg-[#F9F8F6] rounded-full flex items-center justify-center group-hover:bg-[#C15F3C] group-hover:text-white transition-colors border border-[#E5E2DA] flex-shrink-0">
                          <div className="text-[#C15F3C] group-hover:text-white w-4 h-4">
                            {renderIcon(feature.icon)}
                          </div>
                        </div>
                        <span className="text-sm font-semibold text-[#2F2E2B] group-hover:text-[#C15F3C] transition-colors">
                          {feature.text}
                        </span>
                      </div>
                    ))}
