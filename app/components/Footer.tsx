    ...
    <footer className="bg-[#F4F3EE] border-t border-[#E5E2DA]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-[#E5E2DA] overflow-hidden">
              ...
              {footerColumns.map((column, idx) => (
                <div key={idx}>
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h4 className="text-sm font-semibold text-[#2F2E2B] uppercase tracking-wide">
                      {column.title}
                    </h4>
                  </div>
                  <ul className="space-y-3">
                    {column.links.map((link, linkIdx) => (
                      <li key={linkIdx}>
                        <Link href={link.href}>
                          <span className="group flex items-center gap-2 text-sm text-[#6F6B63] hover:text-[#C15F3C] transition-colors cursor-pointer">
                            <svg className="w-3.5 h-3.5 opacity-0 group-hover:opacity-100 transition-all group-hover:translate-x-1" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
          ...
          {/* Newsletter Section */}
          <div className="mx-6 mb-6 p-5 bg-gradient-to-r from-[#F4F3EE] to-white rounded-xl border border-[#E5E2DA]">
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
              ...
              <div className="flex flex-1 max-w-md gap-2">
                <input
                  ...
                  className="flex-1 px-4 py-2.5 bg-white border border-[#E5E2DA] rounded-xl text-sm text-[#2F2E2B] placeholder-[#B1ADA1] focus:outline-none focus:ring-1 focus:ring-[#C15F3C]"
                />
                <button className="px-5 py-2.5 bg-[#C15F3C] hover:bg-[#A94E30] text-white text-sm font-medium rounded-xl transition-all shadow-sm hover:shadow-md whitespace-nowrap">
                  Subscribe
                </button>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              {socialLinks.map((social, idx) => (
                <a
                  ...
                  className="group w-9 h-9 rounded-xl bg-white border border-[#E5E2DA] flex items-center justify-center text-[#6F6B63] hover:bg-[#C15F3C] hover:text-white hover:border-[#C15F3C] transition-all duration-200"
                >
                  {social.icon}
                </a>
              ))}
        ...
        {/* Trust Badge */}
        <div className="mt-6 pt-6 border-t border-[#E5E2DA] text-center">
          <div className="flex flex-wrap items-center justify-center gap-4 text-xs text-[#B1ADA1]">
            <span>✓ 100% Secure</span>
            <span className="w-1 h-1 rounded-full bg-[#E5E2DA]"></span>
