...
const FALLBACK_PACKAGES = [
  { label: "Standard Objection Reply – ₹4,999", amount: 4999 },
  { label: "Urgent Objection Reply – ₹7,499", amount: 7499 },
];
...
interface CopyrightObjectionHeroProps {
  heading?: string;
...
export default function CopyrightObjectionHero({
  heading = "Copyright",
  headingHighlight = "Objection Reply",
  description = "Filing a professional reply to the copyright objection raised by the examiner. Secure your work registration with expert drafting.",
  features = [],
}: CopyrightObjectionHeroProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    phone_no: "",
    email: "",
    copyright_objection_type: FALLBACK_PACKAGES[0]?.label || "",
    cc_number: "",
    reciept_number: "",
    dairy_number: "",
  });
  ...
  const selectedPackage = FALLBACK_PACKAGES.find(p => p.label === formData.copyright_objection_type) || FALLBACK_PACKAGES[0];
  ...
  const handlePayment = async () => {
    ...
    try {
      const res = await fetch("/api/create-order", {
        ...
        body: JSON.stringify({ amount, packageName: "Copyright Objection Reply" }),
      });
      ...
      const options = {
        ...
        currency: "INR",
        name: "DoStartup",
        description: `Copyright Objection - ${selectedPackage.label}`,
        order_id: orderId,
        handler: async (response: any) => {
          try {
            ...
            // Save to Supabase
            const { error: dbError } = await supabase.from("copyright_objection").insert({
              name: formData.name,
              email: finalEmail,
              phone_no: formData.phone_no,
              copyright_objection_type: selectedPackage.label,
              cc_number: formData.cc_number,
              reciept_number: formData.reciept_number,
              dairy_number: formData.dairy_number || null,
              amount: amount,
            ...
            // WhatsApp Message
            const whatsappMsg = `🔔 *New Copyright Objection Reply*\n\n👤 *Name:* ${formData.name}\n📞 *Phone:* ${formData.phone_no}\n📧 *Email:* ${finalEmail}\n📑 *CC No:* ${formData.cc_number}\n📝 *Receipt No:* ${formData.reciept_number}\n💰 *Payment ID:* ${response.razorpay_payment_id}\n\nI have successfully completed the payment. Please proceed with my objection reply.`;
        ...
        <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }}>
          ...
          <div className="bg-white rounded-3xl p-8 border border-[#E5E2DA] shadow-sm">
            <h3 className="text-xs font-bold text-[#B1ADA1] uppercase tracking-wider mb-6 flex items-center gap-2">
              <span className="w-8 h-[1px] bg-[#E5E2DA]"></span> Key Benefits
            </h3>
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="relative">
          <div className="bg-white rounded-3xl p-8 shadow-2xl border border-[#E5E2DA] relative z-10">
            <div className="flex justify-between items-center mb-8">
              <h2 className="text-xl font-bold text-[#2F2E2B]">Start Reply</h2>
            ...
            <AnimatePresence mode="wait">
              ...
              {step === 2 && (
                <motion.div key="step2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-5">
                  <div>
                    <label className="block text-xs text-[#6F6B63] mb-1">Objection Type *</label>
                    <select name="copyright_objection_type" value={formData.copyright_objection_type} onChange={handleInputChange} className="w-full border border-[#E5E2DA] rounded-lg px-4 py-3 text-sm text-[#2F2E2B] focus:ring-1 focus:ring-[#C15F3C] bg-white appearance-none cursor-pointer">
                      {FALLBACK_PACKAGES.map(p => <option key={p.label} value={p.label}>{p.label}</option>)}
                    </select>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-xs text-[#6F6B63] mb-1">CC Number *</label>
                      <input type="text" name="cc_number" value={formData.cc_number} onChange={handleInputChange} required placeholder="CC12345" className="w-full border border-[#E5E2DA] rounded-lg px-4 py-3 text-sm text-[#2F2E2B] focus:ring-1 focus:ring-[#C15F3C] bg-white outline-none" />
                    </div>
                    <div>
                      <label className="block text-xs text-[#6F6B63] mb-1">Receipt Number *</label>
                      <input type="text" name="reciept_number" value={formData.reciept_number} onChange={handleInputChange} required placeholder="R123456" className="w-full border border-[#E5E2DA] rounded-lg px-4 py-3 text-sm text-[#2F2E2B] focus:ring-1 focus:ring-[#C15F3C] bg-white outline-none" />
                    </div>
                  </div>
                  <div>
                    <label className="block text-xs text-[#6F6B63] mb-1">Dairy Number (Optional)</label>
                    <input type="text" name="dairy_number" value={formData.dairy_number} onChange={handleInputChange} placeholder="D123/2024" className="w-full border border-[#E5E2DA] rounded-lg px-4 py-3 text-sm text-[#2F2E2B] focus:ring-1 focus:ring-[#C15F3C] bg-white outline-none" />
                  </div>
                  ...
                  <div className="bg-white rounded-2xl p-6 border border-[#E5E2DA] shadow-sm">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div className="space-y-4 text-sm border-l border-[#E5E2DA] pl-6">
                        <h4 className="text-[10px] font-bold text-[#B1ADA1] uppercase tracking-wider">References</h4>
                        <p className="font-bold text-black">CC: {formData.cc_number}</p>
                        <p className="text-black/80">Rec: {formData.reciept_number}</p>
                      </div>
                    </div>
                  </div>
        ...
        {showSuccessModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} className="bg-white rounded-[3rem] p-12 max-w-lg w-full text-center shadow-2xl relative">
              ...
              <h3 className="text-3xl font-bold text-[#2F2E2B] mb-4">Reply Filed!</h3>
              <p className="text-[#6F6B63] mb-10 leading-relaxed">
                Your copyright objection reply request is confirmed. Redirecting to WhatsApp in <span className="font-bold text-[#2F2E2B]">{countdown}s</span>...
              </p>
