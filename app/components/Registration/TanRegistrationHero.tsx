...
const FALLBACK_PACKAGES = [
  { label: "Basic TAN Registration – ₹999", amount: 999 },
  { label: "Standard TAN + TDS Setup – ₹2,499", amount: 2499 },
  { label: "Premium Annual Compliance – ₹7,499", amount: 7499 },
];

export interface Feature { icon: string; text: string; }

export interface TanRegistrationHeroProps {
  heading?: string;
...
export default function TanRegistrationHero({
  heading = "TAN",
  headingHighlight = "Registration",
  description = "Get your Tax Deduction and Collection Account Number (TAN) quickly. Mandatory for anyone responsible for deducting or collecting tax at source (TDS/TCS).",
  features = [],
  buttonText,
}: TanRegistrationHeroProps) {
  ...
  const handlePayment = async () => {
    ...
    try {
      const res = await fetch("/api/create-order", {
        ...
        body: JSON.stringify({ amount, packageName: "TAN Registration" }),
      });
      ...
      const options = {
        ...
        currency: "INR",
        name: "DoStartup",
        description: `TAN Registration – ${selectedPkg.label}`,
        order_id: orderId,
        handler: async (response: any) => {
          try {
            const finalEmail = formData.email || "noemail@tan.com";
            ...
            const { error: dbError } = await supabase.from("tan_registration").insert(insertData);
            ...
            if (dbError) {
              ...
              if (dbError.code === "PGRST204" || dbError.message.includes("user_id")) {
                const { error: fallbackError } = await supabase.from("tan_registration").insert({
                  name: formData.name,
                  email: finalEmail,
                  phone: formData.phone,
                  amount,
            ...
            const msg = `🔔 *New TAN Registration*\n\n👤 *Name:* ${formData.name}\n📞 *Phone:* ${formData.phone}\n📧 *Email:* ${finalEmail}\n📦 *Package:* ${selectedPkg.label}\n💰 *Payment ID:* ${response.razorpay_payment_id}\n\nPlease proceed with my TAN application.`;
            ...
            fetch("/api/send-confirmation", {
              ...
              body: JSON.stringify({
                ...
                service: "TAN Registration", paymentId: response.razorpay_payment_id,
        ...
        <div className="flex flex-col lg:flex-row gap-8">
            ...
            <div className="flex flex-col md:flex-row gap-8">
              <div className="w-full md:w-64 flex-shrink-0">
                <div className="w-full h-48 bg-gradient-to-br from-[#C15F3C] to-[#A94E30] rounded-xl flex items-center justify-center">
                  <span className="text-white font-bold text-xl text-center px-4">TAN Allocation Service</span>
                </div>
                <div className="mt-4 space-y-2">
                  {["New TAN Application", "TAN Correction", "Duplicate TAN"].map(item => (
              ...
              <div className="flex-1 space-y-6">
                <div className="inline-flex items-center gap-2 bg-white border border-[#E5E2DA] rounded-full px-3 py-1">
                  ...
                  <span className="text-xs font-medium text-[#C15F3C]">OFFICIAL FILING</span>
                </div>
                <div>
          ...
          <div id="registration-form" className="lg:w-96 bg-white rounded-2xl shadow-sm border border-[#E5E2DA] overflow-hidden self-start">
            <div className="bg-gradient-to-r from-[#C15F3C] to-[#A94E30] px-6 py-4">
              <h2 className="text-lg font-semibold text-white">Apply for TAN</h2>
              <p className="text-sm text-[#F4F3EE]">Fastest TAN allocation guaranteed</p>
            </div>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-white rounded-3xl shadow-2xl max-w-md w-full overflow-hidden">
              <div className="bg-gradient-to-r from-[#C15F3C] to-[#A94E30] p-8 text-center relative">
                ...
                <h3 className="text-2xl font-bold text-white mb-1">Application Received!</h3>
              </div>
              <div className="p-8 space-y-6">
                <div className="text-center space-y-2">
                  <p className="text-[#2F2E2B] font-medium text-lg">Thank you, {formData.name}!</p>
                  <p className="text-[#6F6B63] text-sm">Your TAN application has been initiated. Our team will contact you for documentation.</p>
                </div>
                <div className="bg-[#F9F8F6] rounded-2xl p-4 border border-[#E5E2DA] space-y-2 text-xs">
                  <div className="flex justify-between"><span className="text-[#6F6B63]">Service</span><span className="font-semibold">TAN Registration</span></div>
