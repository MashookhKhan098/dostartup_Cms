...
export type TrademarkInfringementHeroProps = {
  heading?: string;
...
const FALLBACK_PACKAGES = [
  { label: "Legal Notice Drafting – ₹2,999", amount: 2999 },
  { label: "Drafting & Formal Dispatch – ₹4,999", amount: 4999 },
];

export default function TrademarkInfringementHero({
  heading,
  headingHighlight,
  description,
  features,
  buttonText,
}: TrademarkInfringementHeroProps) {
  ...
  useEffect(() => {
    ...
    const fetchPricing = async () => {
      try {
        const filter = JSON.stringify({ category: { "$regex": "^trademark-infringement-notice$", "$options": "i" } });
  ...
  const handlePayment = async () => {
    ...
    try {
      const res = await fetch("/api/create-order", {
        ...
        body: JSON.stringify({ amount, packageName: "TM Infringement Notice" }),
      });
      ...
      const options = {
        ...
        currency: "INR",
        name: "DoStartup",
        description: "TM Infringement Notice Drafting",
        order_id: orderId,
        handler: async function (response: any) {
          ...
          const whatsappMsg =
            `🔔 *New TM Infringement Notice Order*\n\n` +
            `👤 *Name:* ${formData.name}\n` +
            `📞 *Phone:* ${formData.phone}\n` +
            ...
            `I have successfully completed the payment. Please proceed with my TM Infringement Notice drafting.`;
          ...
          try {
            ...
            const { error: dbError } = await supabase.from("trademark_infringement_notice").insert({
              name: formData.name,
              email: finalEmail,
              phone_no: formData.phone,
              service: "TM Infringement Notice",
            ...
            console.error("Failed to insert TM Infringement Notice record:", dbErr);
          }
        ...
        <div className="flex flex-col lg:flex-row gap-8">
            ...
            <div className="flex flex-col md:flex-row gap-8">
              <div className="w-full md:w-64 flex-shrink-0">
                <div className="w-full h-48 bg-gradient-to-br from-[#C15F3C] to-[#A94E30] rounded-xl border border-[#E5E2DA] flex items-center justify-center text-center px-4">
                  <span className="text-white font-bold text-xl">TM Infringement Notice</span>
                </div>
                <div className="mt-4 space-y-2">
                  {["Brand Protection", "Legal Notice Drafting", "Enforce Rights"].map((item) => (
              ...
              <div className="flex-1 space-y-6">
                <div className="inline-flex items-center gap-2 bg-white border border-[#E5E2DA] rounded-full px-3 py-1">
                  ...
                  <span className="text-xs font-medium text-[#C15F3C]">LEGAL ENFORCEMENT EXPERTS</span>
                </div>
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <button onClick={() => handleNeedHelpWhatsApp(headingHighlight || heading || "TM Infringement Notice")} className="text-[#C15F3C] hover:underline">Need Help?</button>
                </div>
              </div>
            </div>
          </div>
          ...
          <div id="registration-form" className="lg:w-96 bg-white rounded-2xl shadow-sm border border-[#E5E2DA] overflow-hidden self-start">
            <div className="bg-gradient-to-r from-[#C15F3C] to-[#A94E30] px-6 py-4">
              <h2 className="text-lg font-semibold text-white">Draft Your Notice</h2>
              <p className="text-sm text-[#F4F3EE]">Stop unauthorized use of your brand</p>
            </div>
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }}
              ...
              <div className="bg-gradient-to-r from-[#C15F3C] to-[#A94E30] px-8 py-6 flex justify-between items-center">
                <div>
                  ...
                  <p className="text-white/80 text-xs">Final check before Notice Drafting</p>
                </div>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }}
              ...
              <div className="bg-gradient-to-r from-[#C15F3C] to-[#A94E30] p-8 text-center relative">
                ...
                <h3 className="text-2xl font-bold text-white mb-1">Order Placed Successfully!</h3>
              </div>
              <div className="p-8 space-y-6">
                <div className="space-y-3 text-center">
                  ...
                  <p className="text-[#6F6B63] text-sm leading-relaxed">
                    Your payment of <span className="font-bold text-[#C15F3C]">₹{selectedPackage.amount.toLocaleString()}</span> has been received.
                  </p>
                </div>
                <div className="bg-[#F9F8F6] rounded-2xl p-4 border border-[#E5E2DA] space-y-2">
                  <div className="flex justify-between text-xs">
                    <span className="text-[#6F6B63]">Service</span>
                    <span className="font-semibold text-[#2F2E2B]">TM Infringement Notice</span>
                  </div>
                  <div className="flex justify-between text-xs">
