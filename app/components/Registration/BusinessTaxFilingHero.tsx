...
const FALLBACK_PACKAGES = [
  { label: "Proprietorship ITR Filing – ₹1,499", amount: 1499 },
  { label: "Partnership / LLP Filing – ₹2,999", amount: 2999 },
  { label: "Company Tax Filing – ₹4,999", amount: 4999 },
];

export interface Feature { icon: string; text: string; }

export interface BusinessTaxFilingHeroProps {
  heading?: string;
...
export default function BusinessTaxFilingHero({
  heading = "Business Tax",
  headingHighlight = "Filing",
  description = "File your Business Income Tax Return accurately and on time with expert CA support. Covers Proprietorship, Partnership, LLP, and Company ITR filings.",
  features = [],
  buttonText,
}: BusinessTaxFilingHeroProps) {
  ...
  const handlePayment = async () => {
    ...
    try {
      const res = await fetch("/api/create-order", {
        ...
        body: JSON.stringify({ amount, packageName: "Business Tax Filing" }),
      });
      ...
      const options = {
        ...
        currency: "INR",
        name: "DoStartup",
        description: `Business Tax Filing – ${selectedPkg.label}`,
        order_id: orderId,
        handler: async (response: any) => {
          try {
            const finalEmail = formData.email || "noemail@businesstax.com";
            const { error: dbError } = await supabase.from("bussiness_tax_filing").insert({
              name: formData.name,
              email: finalEmail,
            ...
            const msg = `🔔 *New Business Tax Filing*\n\n👤 *Name:* ${formData.name}\n📞 *Phone:* ${formData.phone_no}\n📧 *Email:* ${finalEmail}\n🏢 *GSTIN:* ${formData.gstin.toUpperCase()}\n📦 *Package:* ${selectedPkg.label}\n💰 *Payment ID:* ${response.razorpay_payment_id}\n\nPlease proceed with my Business Tax Return filing.`;
            ...
            fetch("/api/send-confirmation", {
              ...
              body: JSON.stringify({
                ...
                service: "Business Tax Filing", paymentId: response.razorpay_payment_id,
                details: { "GSTIN": formData.gstin.toUpperCase(), "Package": selectedPkg.label },
              }),
            }).catch(console.error);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (step === 1) {
      ...
      if (!formData.gstin || formData.gstin.length !== 15) {
        alert("Please enter a valid 15-character GSTIN."); return;
      }
      setStep(3);
    }
  };
        ...
        <div className="flex flex-col lg:flex-row gap-8">
            ...
            <div className="flex flex-col md:flex-row gap-8">
              <div className="w-full md:w-64 flex-shrink-0">
                <div className="w-full h-48 bg-gradient-to-br from-[#C15F3C] to-[#A94E30] rounded-xl flex items-center justify-center">
                  <span className="text-white font-bold text-xl text-center px-4">Business Tax Filing</span>
                </div>
                <div className="mt-4 space-y-2">
                  {["ITR-3 / ITR-4 / ITR-5 / ITR-6", "Proprietorship & Company", "GST & TDS Compliance"].map(item => (
          ...
          <div id="registration-form" className="lg:w-96 bg-white rounded-2xl shadow-sm border border-[#E5E2DA] overflow-hidden self-start">
            <div className="bg-gradient-to-r from-[#C15F3C] to-[#A94E30] px-6 py-4">
              <h2 className="text-lg font-semibold text-white">File Business ITR</h2>
              <p className="text-sm text-[#F4F3EE]">Expert CA-assisted business tax filing</p>
            </div>
            ...
            <p className="px-6 pt-1 pb-0 text-[11px] text-[#B1ADA1]">
              {step === 1 ? "Step 1 of 3 – Contact Details" : step === 2 ? "Step 2 of 3 – GSTIN & Package" : "Step 3 of 3 – Review & Pay"}
            </p>
                ...
                <AnimatePresence mode="wait">
                  ...
                  {step === 2 && (
                    <motion.div key="s2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-4">
                      <div>
                        <label className="block text-xs text-[#6F6B63] mb-1">GSTIN *</label>
                        <input type="text" name="gstin" value={formData.gstin} onChange={handleChange} required maxLength={15} placeholder="22AAAAA0000A1Z5" className="w-full border border-[#E5E2DA] rounded-lg px-4 py-3 text-sm text-[#2F2E2B] uppercase focus:outline-none focus:ring-1 focus:ring-[#C15F3C] bg-white" />
                        <p className="text-[10px] text-[#B1ADA1] mt-1">15-character GST Identification Number</p>
                      </div>
                      <div>
                  ...
                  {step === 3 && (
                    <motion.div key="s3" initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} className="space-y-4">
                      <div className="bg-[#F9F8F6] rounded-xl p-4 border border-[#E5E2DA] space-y-2 text-sm">
                        {[["Name", formData.name], ["Email", formData.email], ["Phone", formData.phone_no], ["GSTIN", formData.gstin.toUpperCase()]].map(([l, v]) => (
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-white rounded-3xl shadow-2xl max-w-md w-full overflow-hidden">
              ...
              <div className="p-8 space-y-6">
                <div className="text-center space-y-2">
                  ...
                  <p className="text-[#6F6B63] text-sm">Your business tax filing has been initiated. Our CA team will reach out shortly.</p>
                </div>
                <div className="bg-[#F9F8F6] rounded-2xl p-4 border border-[#E5E2DA] space-y-2 text-xs">
                  <div className="flex justify-between"><span className="text-[#6F6B63]">Service</span><span className="font-semibold">Business Tax Filing</span></div>
