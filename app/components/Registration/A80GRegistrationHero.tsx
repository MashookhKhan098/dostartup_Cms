      ...
      const options = {
        ...
        handler: async function (response: any) {
          ...
          try {
            ...
            // Assuming table name is a80g_registration or matching a12 style
            const { error: dbError } = await supabase.from("a80g_registration").insert({
              name: formData.name,
              email: finalEmail,
              phone_no: formData.phone,
            ...
            if (dbError) {
              ...
              // Fallback for different table name if suspected
              if (dbError.message.includes("does not exist")) {
                 await supabase.from("80g_registration").insert({
                  name: formData.name,
                  email: finalEmail,
                  phone_no: formData.phone,
                  pan_gstin: formData.pan_gstin.toUpperCase(),
