...
const FALLBACK_PACKAGES = [
  { label: "Basic ITR-5 Filing – ₹4,999", amount: 4999 },
  { label: "Standard ITR-5 Filing – ₹7,499", amount: 7499 },
  { label: "Premium ITR-5 Filing – ₹9,999", amount: 9999 },
];

export interface Feature { icon: string; text: string; }

export interface ITR5ReturnHeroProps {
  heading?: string;
...
export default function ITR5ReturnHero({
  heading = "ITR-5",
  headingHighlight = "Return Filing",
  description = "File your ITR-5 return for Partnership Firms, LLPs, AOPs, and BOIs. Comprehensive CA-assisted filing with full audit and compliance support.",
  features = [],
  buttonText,
}: ITR5ReturnHeroProps) {
  ...
  const handlePayment = async () => {
    ...
    try {
      const res = await fetch("/api/create-order", {
        ...
        body: JSON.stringify({ amount, packageName: "ITR-5 Return Filing" }),
      });
      ...
      const options = {
        ...
        currency: "INR",
        name: "DoStartup",
        description: `ITR-5 Return Filing – ${selectedPkg.label}`,
        order_id: orderId,
        handler: async (response: any) => {
          try {
            const finalEmail = formData.email || "noemail@itr5.com";
            ...
            const { error: dbError } = await supabase.from("itr5_return").insert(insertData);
            ...
            if (dbError) {
              ...
              if (dbError.code === "PGRST204" || dbError.message.includes("user_id")) {
                const { error: fallbackError } = await supabase.from("itr5_return").insert({
                  name: formData.name,
                  email: finalEmail,
                  phone_no: formData.phone_no,
                  amount,
            ...
            const msg = `🔔 *New ITR-5 Return Filing*\n\n👤 *Name:* ${formData.name}\n📞 *Phone:* ${formData.phone_no}\n📧 *Email:* ${finalEmail}\n📦 *Package:* ${selectedPkg.label}\n💰 *Payment ID:* ${response.razorpay_payment_id}\n\nPlease proceed with my ITR-5 filing.`;
            ...
            fetch("/api/send-confirmation", {
              ...
              body: JSON.stringify({
                ...
                service: "ITR-5 Return Filing", paymentId: response.razorpay_payment_id,
        ...
        <div className="flex flex-col lg:flex-row gap-8">
            ...
            <div className="flex flex-col md:flex-row gap-8">
              <div className="w-full md:w-64 flex-shrink-0">
                <div className="w-full h-48 bg-gradient-to-br from-[#C15F3C] to-[#A94E30] rounded-xl flex items-center justify-center">
                  <span className="text-white font-bold text-xl text-center px-4">ITR-5 Firm & LLP Filing</span>
                </div>
                <div className="mt-4 space-y-2">
                  {["Partnership Firms", "Limited Liability Partnerships", "AOPs / BOIs"].map(item => (
          ...
          <div id="registration-form" className="lg:w-96 bg-white rounded-2xl shadow-sm border border-[#E5E2DA] overflow-hidden self-start">
            <div className="bg-gradient-to-r from-[#C15F3C] to-[#A94E30] px-6 py-4">
              <h2 className="text-lg font-semibold text-white">File ITR-5 Now</h2>
                  ...
                  {step === 2 && (
                    <motion.div key="s2" initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} className="space-y-4">
                      <div className="bg-[#F9F8F6] rounded-xl p-4 border border-[#E5E2DA] space-y-2 text-sm">
                        {[["Name", formData.name], ["Email", formData.email], ["Phone", formData.phone_no], ["Package", selectedPkg.label.split("–")[0]?.trim() ?? selectedPkg.label]].map(([l, v]) => (
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-white rounded-3xl shadow-2xl max-w-md w-full overflow-hidden">
              ...
              <div className="p-8 space-y-6">
                <div className="text-center space-y-2">
                  ...
                  <p className="text-[#6F6B63] text-sm">Your ITR-5 filing has been initiated. Our CA team will reach out shortly.</p>
                </div>
                <div className="bg-[#F9F8F6] rounded-2xl p-4 border border-[#E5E2DA] space-y-2 text-xs">
                  <div className="flex justify-between"><span className="text-[#6F6B63]">Service</span><span className="font-semibold">ITR-5 Return Filing</span></div>
