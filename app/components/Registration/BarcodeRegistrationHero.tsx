...
export type BarcodeRegistrationHeroProps = {
  heading?: string;
...
export default function BarcodeRegistrationHero({
  heading,
  headingHighlight,
  description,
  features,
  buttonText,
}: BarcodeRegistrationHeroProps) {
  ...
  const handlePayment = async () => {
    ...
    try {
      const res = await fetch("/api/create-order", {
        ...
        body: JSON.stringify({ amount, packageName: "Barcode Registration" }),
      });
      ...
      const options = {
        ...
        handler: async function (response: any) {
          ...
          const whatsappMsg =
            ...
            `I have successfully completed the payment. Please proceed with my Barcode registration.`;

          const waUrl = `https://wa.me/${process.env.NEXT_PUBLIC_WHATSAPP_NUMBER || "919999644807"}?text=${encodeURIComponent(whatsappMsg)}`;

          // Save to Supabase
          try {
            const finalEmail = user?.email || formData.email || "noemail@barcode.com";
            const { error: dbError } = await supabase.from("barcode_registration").insert({
              name: formData.name,
              email: finalEmail,
              phone_no: formData.phone,
            ...
            console.error("Failed to insert Barcode record:", dbErr);
          }
        ...
        <div className="flex flex-col lg:flex-row gap-8">
            ...
            <div className="flex flex-col md:flex-row gap-8">
              <div className="w-full md:w-64 flex-shrink-0">
                <div className="w-full h-48 bg-gradient-to-br from-[#C15F3C] to-[#A94E30] rounded-xl border border-[#E5E2DA] flex items-center justify-center">
                  <span className="text-white font-bold text-xl text-center px-4">Barcode Registration</span>
                </div>
                <div className="mt-4 space-y-2">
                  {["Product Recognition", "Authenticity", "Global Compliance"].map((item) => (
              ...
              <div className="flex-1 space-y-6">
                <div>
                  <div className="inline-flex items-center gap-2 bg-white border border-[#E5E2DA] rounded-full px-3 py-1">
                    ...
                    <span className="text-xs font-medium text-[#C15F3C]">CERTIFIED GS1 ASSISTANCE</span>
                  </div>
                </div>
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <button
                    onClick={() => handleNeedHelpWhatsApp(headingHighlight || heading || "Barcode Registration")}
          ...
          <div id="registration-form" className="lg:w-96 bg-white rounded-2xl shadow-sm border border-[#E5E2DA] overflow-hidden self-start">
            <div className="bg-gradient-to-r from-[#C15F3C] to-[#A94E30] px-6 py-4">
              <h2 className="text-lg font-semibold text-white">Apply for Barcode</h2>
              <p className="text-sm text-[#F4F3EE]">Official GS1 Barcode Services</p>
            </div>
            ...
            <div className="p-6 space-y-6">
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="flex items-center justify-center gap-2 text-xs text-[#B1ADA1] pt-2">
                  ...
                  <span>Secure Gateway · Professional Support</span>
                </div>
              </form>
            </div>
          </div>
            ...
            >
              <div className="bg-gradient-to-r from-[#C15F3C] to-[#A94E30] px-8 py-6 flex justify-between items-center">
                <div>
                  ...
                  <p className="text-white/80 text-xs">Verify your product identification info</p>
                </div>
              ...
              <div className="p-8 space-y-6">
                ...
                <div className="bg-[#F9F8F6] rounded-2xl p-6 border border-[#E5E2DA] flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-[#2F2E2B]">₹8,999 <span className="text-xs font-normal text-[#B1ADA1]">incl. taxes</span></p>
                  </div>
                </div>
                ...
                <div className="flex gap-4">
                  ...
                  >
                    {loading ? (
                      ...
                      <>{buttonText || "Pay INR 8,999 Online"} →</>
                    )}
                  </button>
                </div>
              </div>
              ...
              <div className="p-8 space-y-6">
                <div className="space-y-3 text-center">
                  ...
                  <p className="text-[#6F6B63] text-sm leading-relaxed">
                    Your payment of <span className="font-bold text-[#C15F3C]">₹8,999</span> has been received successfully.
                  </p>
                </div>
                <div className="bg-[#F9F8F6] rounded-2xl p-4 border border-[#E5E2DA] space-y-2">
                  <div className="flex justify-between text-xs">
                    <span className="text-[#6F6B63]">Service</span>
                    <span className="font-semibold text-[#2F2E2B]">Barcode Registration</span>
                  </div>
                  <div className="flex justify-between text-xs">
