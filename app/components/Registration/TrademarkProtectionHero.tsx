...
const FALLBACK_PACKAGES = [
  { label: "Trademark Monitoring – ₹4,999", amount: 4999 },
  { label: "TM Watch Service – ₹9,999", amount: 9999 },
  { label: "Infringement Protection – ₹14,999", amount: 14999 },
];
...
interface TrademarkProtectionHeroProps {
  heading?: string;
...
export default function TrademarkProtectionHero({
  heading = "Trademark",
  headingHighlight = "Protection",
  description = "Protect your brand from unauthorized use and infringement. Our monitoring services ensure your IP remains exclusively yours.",
  features = [],
}: TrademarkProtectionHeroProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    phone_no: "",
    email: "",
    service_type: FALLBACK_PACKAGES[0]?.label || "",
    application_number: "",
    brand_name: "",
    trademark_class: "",
  });
  ...
  const selectedPackage = FALLBACK_PACKAGES.find(p => p.label === formData.service_type) || FALLBACK_PACKAGES[0];
  ...
  const handlePayment = async () => {
    ...
    try {
      const res = await fetch("/api/create-order", {
        ...
        body: JSON.stringify({ amount, packageName: "Trademark Protection" }),
      });
      ...
      const options = {
        ...
        currency: "INR",
        name: "DoStartup",
        description: `TM Protection - ${selectedPackage.label}`,
        order_id: orderId,
        handler: async (response: any) => {
          try {
            const finalEmail = formData.email || "noemail@trademark.com";
            
            const { error: dbError } = await supabase.from("trademark_protection").insert({
              name: formData.name || null,
              email: finalEmail,
              phone_no: formData.phone_no || null,
              service_type: selectedPackage.label,
              application_number: formData.application_number,
              brand_name: formData.brand_name,
              trademark_class: formData.trademark_class,
              amount: amount,
            ...
            const whatsappMsg = `🔔 *New Trademark Protection*\n\n👤 *Name:* ${formData.name}\n📞 *Phone:* ${formData.phone_no}\n🔖 *Brand:* ${formData.brand_name}\n🔢 *App No:* ${formData.application_number}\n📦 *Class:* ${formData.trademark_class}\n💰 *Payment ID:* ${response.razorpay_payment_id}\n\nI have successfully completed the payment. Please start my protection service.`;
        ...
        <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }}>
          ...
          <div className="bg-white rounded-3xl p-8 border border-[#E5E2DA] shadow-sm">
            <h3 className="text-xs font-bold text-[#B1ADA1] uppercase tracking-wider mb-6 flex items-center gap-2">
              <span className="w-8 h-[1px] bg-[#E5E2DA]"></span> Why Protect?
            </h3>
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="relative">
          <div className="bg-white rounded-3xl p-8 shadow-2xl border border-[#E5E2DA] relative z-10">
            <div className="flex justify-between items-center mb-8">
              <h2 className="text-xl font-bold text-[#2F2E2B]">Protection Setup</h2>
              ...
              {step === 1 && (
                <motion.div key="step1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-5">
                  <div>
                    ...
                    <input type="text" name="name" value={formData.name} onChange={handleInputChange} required placeholder="Applicant Name" className="w-full border border-[#E5E2DA] rounded-lg px-4 py-3 text-sm text-[#2F2E2B] focus:ring-1 focus:ring-[#C15F3C] bg-white outline-none" />
                  </div>
                  {!isLoggedIn && (
                    <div>
                ...
                <motion.div key="step2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-5">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input type="text" name="brand_name" value={formData.brand_name} onChange={handleInputChange} required placeholder="e.g. DoStartup" className="w-full border border-[#E5E2DA] rounded-lg px-4 py-3 text-sm text-[#2F2E2B] focus:ring-1 focus:ring-[#C15F3C] bg-white outline-none" />
                    </div>
                    <div>
                      <label className="block text-xs text-[#6F6B63] mb-1">TM Class *</label>
                      <input type="text" name="trademark_class" value={formData.trademark_class} onChange={handleInputChange} required placeholder="e.g. Class 35" className="w-full border border-[#E5E2DA] rounded-lg px-4 py-3 text-sm text-[#2F2E2B] focus:ring-1 focus:ring-[#C15F3C] bg-white outline-none" />
                    </div>
                  </div>
                  <div>
                    <label className="block text-xs text-[#6F6B63] mb-1">Application Number *</label>
                    <input type="text" name="application_number" value={formData.application_number} onChange={handleInputChange} required placeholder="8-digit TM Application No" className="w-full border border-[#E5E2DA] rounded-lg px-4 py-3 text-sm text-[#2F2E2B] focus:ring-1 focus:ring-[#C15F3C] bg-white outline-none" />
                  </div>
                  <div>
                    <label className="block text-xs text-[#6F6B63] mb-1">Service Type *</label>
                    <select name="service_type" value={formData.service_type} onChange={handleInputChange} className="w-full border border-[#E5E2DA] rounded-lg px-4 py-3 text-sm text-[#2F2E2B] focus:ring-1 focus:ring-[#C15F3C] bg-white appearance-none cursor-pointer">
              ...
              {step === 3 && (
                <motion.div key="step3" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="space-y-6">
                  <div className="bg-white rounded-2xl p-6 border border-[#E5E2DA] shadow-sm">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="space-y-4 text-sm">
                        <h4 className="text-[10px] font-bold text-[#B1ADA1] uppercase tracking-wider">Applicant</h4>
                      ...
                      <div className="space-y-4 text-sm border-l border-[#E5E2DA] pl-6">
                        ...
                        <p className="font-bold text-black uppercase tracking-tight">{formData.brand_name}</p>
                        <p className="text-black/80">App No: {formData.application_number}</p>
                        <p className="text-black/80">{formData.trademark_class}</p>
                      </div>
                    </div>
                  </div>
                  <div className="bg-[#F9F8F6] rounded-2xl p-6 border border-[#E5E2DA] flex justify-between items-center">
                    ...
                    <span className="text-[10px] text-[#A94E30] font-bold uppercase">Guaranteed Security</span>
                  </div>
        ...
        {showSuccessModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} className="bg-white rounded-[3rem] p-12 max-w-lg w-full text-center shadow-2xl relative">
              <button onClick={() => { setShowSuccessModal(false); setCountdown(-1); }} className="absolute top-8 right-8 text-[#B1ADA1] hover:text-[#2F2E2B] transition-colors">
              ...
              <h3 className="text-3xl font-bold text-[#2F2E2B] mb-4">Protection Active!</h3>
              <p className="text-[#6F6B63] mb-10 leading-relaxed">
                Your monitoring for <span className="text-[#C15F3C] font-bold">{formData.brand_name}</span> is confirmed. Redirecting to WhatsApp in <span className="font-bold text-[#2F2E2B]">{countdown}s</span>...
              </p>
              <button onClick={() => window.open(whatsappUrl, "_blank")} className="w-full bg-[#25D366] text-white font-bold py-5 rounded-2xl flex items-center justify-center gap-3 shadow-lg shadow-green-200 active:scale-95 transition-transform">
                Connect on WhatsApp
              </button>
            </motion.div>
          </motion.div>
