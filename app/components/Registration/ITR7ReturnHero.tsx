...
const FALLBACK_PACKAGES = [
  { label: "Basic ITR-7 Filing – ₹14,999", amount: 14999 },
  { label: "Standard ITR-7 Filing – ₹19,999", amount: 19999 },
  { label: "Premium ITR-7 Filing – ₹24,999", amount: 24999 },
];

export interface Feature { icon: string; text: string; }

export interface ITR7ReturnHeroProps {
  heading?: string;
...
export default function ITR7ReturnHero({
  heading = "ITR-7",
  ...
  const handlePayment = async () => {
    ...
    try {
      const res = await fetch("/api/create-order", {
        ...
        body: JSON.stringify({ amount, packageName: "ITR-7 Return Filing" }),
      });
      ...
      const options = {
        ...
        currency: "INR",
        name: "DoStartup",
        description: `ITR-7 Return Filing – ${selectedPkg.label}`,
        order_id: orderId,
        handler: async (response: any) => {
          try {
            const finalEmail = formData.email || "noemail@itr7.com";
            const { error: dbError } = await supabase.from("itr7_return").insert({
              name: formData.name,
              email: finalEmail,
            ...
            const msg = `🔔 *New ITR-7 Return Filing*\n\n👤 *Trust/NGO Name:* ${formData.name}\n📞 *Phone:* ${formData.phone_no}\n📧 *Email:* ${finalEmail}\n📦 *Package:* ${selectedPkg.label}\n💰 *Payment ID:* ${response.razorpay_payment_id}\n\nPlease proceed with my ITR-7 filing.`;
            ...
            fetch("/api/send-confirmation", {
              ...
              body: JSON.stringify({
                ...
                service: "ITR-7 Return Filing", paymentId: response.razorpay_payment_id,
        ...
        <div className="flex flex-col lg:flex-row gap-8">
            ...
            <div className="flex flex-col md:flex-row gap-8">
              <div className="w-full md:w-64 flex-shrink-0">
                <div className="w-full h-48 bg-gradient-to-br from-[#C15F3C] to-[#A94E30] rounded-xl flex items-center justify-center">
                  <span className="text-white font-bold text-xl text-center px-4">ITR-7 NGO & Trust Filing</span>
                </div>
                <div className="mt-4 space-y-2">
                  {["Charitable Trusts", "NGOs / Societies", "Political Parties", "Institutions"].map(item => (
          ...
          <div id="registration-form" className="lg:w-96 bg-white rounded-2xl shadow-sm border border-[#E5E2DA] overflow-hidden self-start">
            <div className="bg-gradient-to-r from-[#C15F3C] to-[#A94E30] px-6 py-4">
              <h2 className="text-lg font-semibold text-white">File ITR-7 Now</h2>
                ...
                <AnimatePresence mode="wait">
                  {step === 1 && (
                    <motion.div key="s1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-4">
                      <div>
                        <label className="block text-xs text-[#6F6B63] mb-1">Entity Name *</label>
                        <input type="text" name="name" value={formData.name} onChange={handleChange} required placeholder="Trust / NGO / Party name" className="w-full border border-[#E5E2DA] rounded-lg px-4 py-3 text-sm text-[#2F2E2B] focus:outline-none focus:ring-1 focus:ring-[#C15F3C] bg-white" />
                      </div>
                      {!isLoggedIn && (
                        <div>
                  ...
                  {step === 2 && (
                    <motion.div key="s2" initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} className="space-y-4">
                      <div className="bg-[#F9F8F6] rounded-xl p-4 border border-[#E5E2DA] space-y-2 text-sm">
                        {[["Entity", formData.name], ["Email", formData.email], ["Phone", formData.phone_no], ["Package", selectedPkg.label.split("–")[0]?.trim() ?? selectedPkg.label]].map(([l, v]) => (
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-white rounded-3xl shadow-2xl max-w-md w-full overflow-hidden">
              ...
              <div className="p-8 space-y-6">
                <div className="text-center space-y-2">
                  ...
                  <p className="text-[#6F6B63] text-sm">Your ITR-7 filing has been initiated. Our CA team will reach out shortly.</p>
                </div>
                <div className="bg-[#F9F8F6] rounded-2xl p-4 border border-[#E5E2DA] space-y-2 text-xs">
                  <div className="flex justify-between"><span className="text-[#6F6B63]">Service</span><span className="font-semibold">ITR-7 Return Filing</span></div>
