...
const FALLBACK_PACKAGES = [
  { label: "Basic 15CA Filing – ₹1,999", amount: 1999 },
  { label: "Standard 15CA/15CB (with CA Cert) – ₹4,999", amount: 4999 },
  { label: "Premium Foreign Remittance (Bulk) – ₹9,999", amount: 9999 },
];

export interface Feature { icon: string; text: string; }

export interface Ca15Cb15FilingHeroProps {
  heading?: string;
...
export default function Ca15Cb15FilingHero({
  heading = "15CA & 15CB",
  headingHighlight = "Filing",
  description = "Expert assistance for foreign remittances. We handle Form 15CA (Declaration) and Form 15CB (CA Certificate) to ensure hassle-free global payments.",
  features = [],
  buttonText,
}: Ca15Cb15FilingHeroProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    email: "",
    phone: "",
    package: FALLBACK_PACKAGES[1]!.label, // Default to Standard
  });
  ...
  const selectedPkg = (pricingPackages.find(p => p.label === formData.package) || pricingPackages[1])!;
  ...
  const handlePayment = async () => {
    ...
    try {
      const res = await fetch("/api/create-order", {
        ...
        body: JSON.stringify({ amount, packageName: "15CA-15CB Filing" }),
      });
      ...
      const options = {
        ...
        currency: "INR",
        name: "DoStartup",
        description: `15CA-15CB Filing – ${selectedPkg.label}`,
        order_id: orderId,
        handler: async (response: any) => {
          try {
            const finalEmail = formData.email || "noemail@15ca.com";
            // Attempt robust insert
            ...
            const { error: dbError } = await supabase.from("ca15_cb15_filing").insert(insertData);
            
            if (dbError) {
              console.error("Primary Insert Error (with user_id):", dbError);
              // Fallback: If user_id column is missing, try without it
              if (dbError.code === "PGRST204" || dbError.message.includes("user_id")) {
                const { error: fallbackError } = await supabase.from("ca15_cb15_filing").insert({
                  name: formData.name,
                  email: finalEmail,
                  phone: formData.phone,
                  amount,
            ...
            const msg = `🔔 *New 15CA-15CB Filing*\n\n👤 *Name:* ${formData.name}\n📞 *Phone:* ${formData.phone}\n📧 *Email:* ${finalEmail}\n📦 *Package:* ${selectedPkg.label}\n💰 *Payment ID:* ${response.razorpay_payment_id}\n\nPlease help me with my 15CA/15CB compliance.`;
            ...
            fetch("/api/send-confirmation", {
              ...
              body: JSON.stringify({
                ...
                service: "15CA-15CB Filing", paymentId: response.razorpay_payment_id,
                details: { "Package": selectedPkg.label },
              }),
            }).catch(console.error);
        ...
        <div className="flex flex-col lg:flex-row gap-8">
            ...
            <div className="flex flex-col md:flex-row gap-8">
              <div className="w-full md:w-64 flex-shrink-0">
                <div className="w-full h-48 bg-gradient-to-br from-[#C15F3C] to-[#A94E30] rounded-xl flex items-center justify-center">
                  <span className="text-white font-bold text-xl text-center px-4">Foreign Remittance Compliance</span>
                </div>
                <div className="mt-4 space-y-2">
                  {["15CA Declaration", "15CB CA Certificate", "DTAA Benefit Advice"].map(item => (
              ...
              <div className="flex-1 space-y-6">
                <div className="inline-flex items-center gap-2 bg-white border border-[#E5E2DA] rounded-full px-3 py-1">
                  ...
                  <span className="text-xs font-medium text-[#C15F3C]">EXPERT ASSISTED</span>
                </div>
                <div>
          ...
          <div id="registration-form" className="lg:w-96 bg-white rounded-2xl shadow-sm border border-[#E5E2DA] overflow-hidden self-start">
            <div className="bg-gradient-to-r from-[#C15F3C] to-[#A94E30] px-6 py-4">
              <h2 className="text-lg font-semibold text-white">File 15CA/15CB Now</h2>
              <p className="text-sm text-[#F4F3EE]">Fast & secure global remittance</p>
            </div>
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-white rounded-3xl shadow-2xl max-w-md w-full overflow-hidden">
              ...
              <div className="p-8 space-y-6">
                <div className="text-center space-y-2">
                  ...
                  <p className="text-[#6F6B63] text-sm">Your 15CA/15CB filing has been initiated. Our team will contact you shortly.</p>
                </div>
                <div className="bg-[#F9F8F6] rounded-2xl p-4 border border-[#E5E2DA] space-y-2 text-xs">
                  <div className="flex justify-between"><span className="text-[#6F6B63]">Service</span><span className="font-semibold">15CA-15CB Filing</span></div>
