...
export type DigitalSignatureHeroProps = {
  heading?: string;
...
const FALLBACK_PACKAGES = [
  { label: "Class 3 Individual – ₹1,299", amount: 1299 },
  { label: "Class 3 Organization – ₹2,499", amount: 2499 },
  { label: "Class 3 with Encryption – ₹3,499", amount: 3499 },
];

export default function DigitalSignatureHero({
  heading,
  headingHighlight,
  description,
  features,
  buttonText,
}: DigitalSignatureHeroProps) {
  ...
  useEffect(() => {
    ...
    const fetchPricing = async () => {
      try {
        const filter = JSON.stringify({ category: { "$regex": "^digital-signature$", "$options": "i" } });
  ...
  const handlePayment = async () => {
    ...
    try {
      const res = await fetch("/api/create-order", {
        ...
        body: JSON.stringify({ amount, packageName: "Digital Signature Certificate" }),
      });
      ...
      const options = {
        ...
        handler: async function (response: any) {
          ...
          const whatsappMsg =
            `🔔 *New Digital Signature Certificate Request*\n\n` +
            `👤 *Name:* ${formData.name}\n` +
            `📞 *Phone:* ${formData.phone}\n` +
            `📧 *Email:* ${formData.email || "Not provided"}\n` +
            `📦 *Package:* ${formData.package}\n` +
            `💰 *Payment ID:* ${response.razorpay_payment_id}\n\n` +
            `I have successfully completed the payment. Please proceed with my DSC application.`;

          const waUrl = `https://wa.me/${process.env.NEXT_PUBLIC_WHATSAPP_NUMBER || "919999644807"}?text=${encodeURIComponent(whatsappMsg)}`;

          // Save to Supabase
          try {
            const finalEmail = user?.email || formData.email || "noemail@dsc.com";
            const { error: dbError } = await supabase.from("digital_signature").insert({
              name: formData.name,
              email: finalEmail,
              phone_no: formData.phone,
              amount: amount,
            ...
            console.error("Failed to insert Digital Signature record:", dbErr);
          }
        ...
        <div className="flex flex-col lg:flex-row gap-8">
            ...
            <div className="flex flex-col md:flex-row gap-8">
              <div className="w-full md:w-64 flex-shrink-0">
                <div className="w-full h-48 bg-gradient-to-br from-[#C15F3C] to-[#A94E30] rounded-xl border border-[#E5E2DA] flex items-center justify-center">
                  <span className="text-white font-bold text-xl text-center px-4">Digital Signature Certificate</span>
                </div>
                <div className="mt-4 space-y-2">
                  {["Class 3 DSC", "eMudhra Partner", "Fast Delivery"].map((item) => (
              ...
              <div className="flex-1 space-y-6">
                <div>
                  <div className="inline-flex items-center gap-2 bg-white border border-[#E5E2DA] rounded-full px-3 py-1">
                    ...
                    <span className="text-xs font-medium text-[#C15F3C]">AUTHORIZED EMUDHRA PARTNER</span>
                  </div>
                </div>
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <button
                    onClick={() => handleNeedHelpWhatsApp(headingHighlight || heading || "Digital Signature Certificate")}
          ...
          <div id="registration-form" className="lg:w-96 bg-white rounded-2xl shadow-sm border border-[#E5E2DA] overflow-hidden self-start">
            <div className="bg-gradient-to-r from-[#C15F3C] to-[#A94E30] px-6 py-4">
              <h2 className="text-lg font-semibold text-white">Apply for DSC</h2>
              <p className="text-sm text-[#F4F3EE]">Digital Signature Certificate services</p>
            </div>
            ...
            >
              <div className="bg-gradient-to-r from-[#C15F3C] to-[#A94E30] px-8 py-6 flex justify-between items-center">
                <div>
                  ...
                  <p className="text-white/80 text-xs">Final check before DSC application</p>
                </div>
              ...
              <div className="p-8 space-y-6">
                ...
                <div className="bg-[#F9F8F6] rounded-2xl p-4 border border-[#E5E2DA] space-y-2">
                  <div className="flex justify-between text-xs">
                    ...
                    <span className="font-semibold text-[#2F2E2B]">Digital Signature Certificate</span>
                  </div>
                  <div className="flex justify-between text-xs">
